...
def sendemail(email,password):
    ...
    sg = sendgrid.SendGridAPIClient(api_key="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")
    from_email = Email("mageshwaranit@gmail.com")  
...
def dashboard():
    ...
    sql = f"""select * from "BLN37196"."REQUEST" Where "UNIQID"!='{uid}' AND "status"='waiting'"""
    ...
    sql2 = f"""select distinct loc from "BLN37196"."REQUEST" Where "UNIQID"!='{uid}' AND "status"='waiting';"""
    sql3 = f"""select distinct hosp from "BLN37196"."REQUEST" Where "UNIQID"!='{uid}' AND "status"='waiting';"""
    sql4 = f"""select distinct bg from "BLN37196"."REQUEST" Where "UNIQID"!='{uid}' AND "status"='waiting';"""
...
def chngstatus(id):
    ...
    sql = f"""UPDATE "BLN37196"."REQUEST" SET "donorid" = '{uid}', "donorname" = '{name}',"status"='accepted' WHERE "FUNIQID" = '{id}';"""
    stmt = ibm_db.prepare(con, sql)
    ibm_db.execute(stmt)
    print("suc")
...
def reqform_post():
    ...
    sql = f"""INSERT INTO  "BLN37196"."REQUEST" ("UNIQID","FUNIQID","NAME","BG","LOC","HOSP","status") VALUES('{uid}','{formid}','{name}','{bg}','{loc}','{hosp}','waiting');"""
...
def login():
    ...
    sql = """SELECT * FROM "BLN37196"."USER_DETAILS" where email = '{usr}' AND passw = '{pas}';""".format(usr=username,pas=password)
    ...
    if(user == username):
        ...
        sql1 = """SELECT * FROM "BLN37196"."USER_DETAILS" where email = '{usr}';""".format(usr=username)
